...
public final class PositionRiskHandler {
    ...
    private volatile long realizedLossPaisa;
    ...
    public void onDomainEvent(DomainEvent event, Consumer<DomainEvent> downstream) {
        ...
        if (event instanceof TradeClosed tradeClosed) {
            ...
            if (tradeClosed.realizedPnlPaisa() < 0) {
                consecutiveLosses++;
                realizedLossPaisa += Math.abs(tradeClosed.realizedPnlPaisa());
            } else {
                consecutiveLosses = 0;
            }
        }
    ...
    private void qualifySignal(SignalGenerated signal, Consumer<DomainEvent> downstream) {
        ...
        if (realizedLossPaisa >= riskLimits.maxDailyLossPaisa() || consecutiveLosses >= riskLimits.maxConsecutiveLosses()) {
            killSwitch = true;
            downstream.accept(new KillSwitchEngaged(EventMetadata.correlated(signal.signalId(), signal.sequenceId()), "Risk limit breached", 0L, 0L, realizedLossPaisa, consecutiveLosses));
        ...
        java.util.List<Instrument> matches = instrumentResolver.allInstruments().stream()
                .filter(candidate -> candidate.canonicalSymbol().equalsIgnoreCase(signal.symbol()) || candidate.symbol().equalsIgnoreCase(signal.symbol()))
                .toList();
        if (matches.isEmpty()) {
