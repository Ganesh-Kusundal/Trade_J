...
public class IciciConfiguration {
    ...
    @Bean
    InstrumentResolver instrumentResolver(IciciBrokerConnection conn) {
        return conn.instruments();
    }

    @Bean
    OrderQuery orderQuery(IciciBrokerConnection conn) {
        return conn.orderQuery();
    }

    @Bean
    PortfolioProvider portfolioProvider(IciciBrokerConnection conn) {
        return conn.portfolio();
    }

    @Bean
    MarginProvider marginProvider(IciciBrokerConnection conn) {
        return conn.margin();
    }

    @Bean
    FuturesProvider futuresProvider(IciciBrokerConnection conn) {
        return conn.futures();
    }

    @Bean
    OptionsProvider optionsProvider(IciciBrokerConnection conn) {
        return conn.options();
    }

    @Bean
    WebSocketMultiplexer webSocketMultiplexer(IciciBrokerConnection conn) {
        return conn.websocket();
    }

    @Bean
    ConditionalAlertProvider conditionalAlertProvider(IciciBrokerConnection conn) {
        return conn.alerts();
    }
