...
import com.tradej.execution.node.*;
...
/**
 * Maps declarative {@link PipelineNodeDef} records to live pipeline node adapters.
 * <p>
 * Supports two lookup paths:
 * <ol>
 *   <li><b>Registry path</b> — if a {@link NodeTypeDescriptor} is registered for the type,
 *       its factory is used directly.</li>
 *   <li><b>Legacy switch path</b> — falls back to the hardcoded type switch for backward
 *       compatibility.</li>
 * </ol>
 * Once all node types are migrated to the registry, the legacy switch path can be removed.
 */
public final class PipelineNodeFactory {
    ...
    /**
     * Creates a runtime node from a declarative definition.
     * <p>
     * Uses the legacy switch-based creation. The {@link NodeRegistry} is used
     * exclusively for metadata/descriptor queries (frontend palette, compiler validation).
     */
