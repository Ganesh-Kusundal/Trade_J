...
// ── Research Platform (Phase 2) ──────────────────────────────────
export interface Experiment {
  experimentId: string;
  name: string;
  hypothesis: string;
  pipelineType: 'REPLAY' | 'SCANNER' | 'STRATEGY';
...
export interface TradeLogEntry {
  ...
  side: string;
  entryTimeMs: number;
  entryPricePaisa: number;
  exitTimeMs: number;
  exitPricePaisa: number;
  quantity: number;
  realizedPnlPaisa: number;
  maePaisa: number | null;
  mfePaisa: number | null;
}
...
export interface SweepTaskRequest {
  sessionId: string;
  strategyName: string;
