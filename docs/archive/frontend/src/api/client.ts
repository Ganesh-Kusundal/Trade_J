...
export const researchApi = {
  // Experiments
  listExperiments: () =>
    request<Experiment[]>('/api/research/experiments'),

  createExperiment: (name: string, hypothesis: string, pipelineType: string, owner: string) =>
    request<Experiment>('/api/research/experiments', {
      method: 'POST',
      body: JSON.stringify({name, hypothesis, pipelineType, owner}),
    }),

  getExperiment: (id: string) =>
    request<Experiment>(`/api/research/experiments/${encodeURIComponent(id)}`),

  // Runs
  getExperimentRuns: (experimentId: string) =>
    request<ExperimentRun[]>(`/api/research/experiments/${encodeURIComponent(experimentId)}/runs`),

  startRun: (experimentId: string, params: Record<string, unknown>) =>
    request<ExperimentRun>(`/api/research/experiments/${encodeURIComponent(experimentId)}/runs`, {
      method: 'POST',
      body: JSON.stringify(params),
    }),

  // Backtest Results (from DuckDB)
  getRunResults: (sessionId?: string) =>
    request<BacktestRunResult[]>(`/api/research/analytics/run-results${sessionId ? `?sessionId=${encodeURIComponent(sessionId)}` : ''}`),

  getTradeLog: (runId: string) =>
    request<TradeLogEntry[]>(`/api/research/analytics/trade-log?runId=${encodeURIComponent(runId)}`),

  getSessions: () =>
    request<string[]>('/api/research/analytics/sessions'),

  // Sweep (Strategy Optimization)
  runSweep: (sweepReq: SweepTaskRequest) =>
    request<SweepResultResponse>('/api/research/lab/sweep', {
      method: 'POST',
  ...
  // Replay
  getReplayStatus: () =>
    request<Record<string, unknown>>('/api/research/replay/status'),

  startReplay: (symbol: string, exchange: string, fromMs: number, toMs: number) =>
    request<Record<string, unknown>>(
      `/api/research/replay/start?symbol=${encodeURIComponent(symbol)}&exchange=${encodeURIComponent(exchange)}&fromMs=${fromMs}&toMs=${toMs}`,
      {method: 'POST'}
    ),

  playReplay: () =>
    request<Record<string, unknown>>('/api/research/replay/play', {method: 'POST'}),

  pauseReplay: () =>
    request<Record<string, unknown>>('/api/research/replay/pause', {method: 'POST'}),

  stepReplay: () =>
    request<Record<string, unknown>>('/api/research/replay/step', {method: 'POST'}),

  stopReplay: () =>
    request<Record<string, unknown>>('/api/research/replay/stop', {method: 'POST'}),

  setReplaySpeed: (speed: number) =>
    request<Record<string, unknown>>(`/api/research/replay/speed?multiplier=${speed}`, {method: 'POST'}),
};
