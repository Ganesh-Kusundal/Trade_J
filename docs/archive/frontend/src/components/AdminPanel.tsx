...
import type {RuntimeInfo, StrategyPluginRow} from '@/dto/types';
import {AlertTriangle, RefreshCw} from 'lucide-react';

export function AdminPanel() {
  const [runtime, setRuntime] = useState<RuntimeInfo | null>(null);
  const [strategies, setStrategies] = useState<StrategyPluginRow[]>([]);
  ...
  const loadAll = () => {
    setLoadError(null);
    adminApi.runtime().then(setRuntime).catch((e) => {
      setLoadError(e instanceof Error ? e.message : String(e));
    });
    adminApi.strategies().then(setStrategies).catch(() => setStrategies([]));
  ...
  const toggleKill = async () => {
    const next = killSwitchEnabled !== true;
    try {
      const res = await adminApi.killSwitch(next);
      setKillSwitchEnabled(res.enabled);
      loadAll();
    } catch (e) {
      setLoadError(e instanceof Error ? e.message : String(e));
    }
  ...
  return (
    <div className="h-full flex flex-col text-[10px] font-mono">
      ...
      <div className="p-3 space-y-3 flex-1 overflow-y-auto">
        {loadError && (
          <div className="bg-[#ef4444]/10 border border-[#ef4444]/30 rounded-xs p-2 text-[#ef4444] text-[9px]">
            {loadError}
          </div>
        )}
        ...
        <div className="flex gap-2 items-center">
          ...
          >
            ...
            {killSwitchEnabled === true ? 'DISABLE KILL SWITCH' : 'ENABLE KILL SWITCH'}
          </button>
          {killSwitchEnabled !== null && (
            <span className={`text-[9px] font-bold ${killSwitchEnabled ? 'text-[#ef4444]' : 'text-[#10b981]'}`}>
        ...
        <div>
          ...
          {strategies.length === 0 && (
            <div className="text-zinc-600 text-[9px]">No strategy plugins loaded</div>
          )}
          {strategies.map((s) => (
            <div key={s.name} className="flex items-center justify-between bg-[#0e0e11] border border-[#1c1c1e] rounded-xs px-2 py-1.5 mb-1">
              <span className="text-zinc-300 font-bold">{s.name}</span>
              <span className="text-[9px] px-1.5 text-zinc-500 bg-zinc-800/50 border border-zinc-700">PLUGIN</span>
            </div>
          ))}
        </div>
              ...
              {Object.entries(summary).map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  ...
                  <span className="text-zinc-200 font-bold">
                    {typeof v === 'number' ? v.toFixed(2) : Array.isArray(v) ? `[${v.length}]` : String(v)}
                  </span>
                </div>
              ))}
            </div>
