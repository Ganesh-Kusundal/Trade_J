...
import {Terminal, Wifi, BarChart3, Search, GitBranch, Settings, FlaskConical, PanelLeftClose, PanelLeft} from 'lucide-react';
...
export default function App() {
  const {
    ...
    sidebarOpen, toggleSidebar, orderBlockZones,
  } = useStudioStore();
        ...
        <div className="flex items-center gap-4 text-[10px]">
          <div className="flex items-center gap-3 border border-zinc-800 bg-[#0e0e11] px-3 h-7 rounded-xs text-zinc-500">
            <div className="flex items-center gap-1.5 font-semibold text-[9px]">
              ...
              <span className="text-zinc-400 uppercase">FEED:</span>
          ...
          <div className="flex items-center gap-1 bg-[#0e0e11] p-0.5 border border-zinc-800 rounded-xs h-7">
            {[
              ...
              {view: 'research' as const, icon: FlaskConical},
          ...
          <button
            ...
            className={`h-7 px-2.5 rounded-xs font-bold text-[9px] transition-colors cursor-pointer border ${
              sidebarOpen
                ? 'bg-zinc-800 text-white border-zinc-700'
                : 'bg-[#0e0e11] text-zinc-500 hover:text-zinc-300 border-zinc-800'
            }`}
          >
            ...
            <div className="flex flex-1 min-h-0">
              <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="absolute top-3 left-4 z-40 bg-[#09090b]/92 border border-zinc-800 rounded-sm p-3 backdrop-blur-xs flex items-center gap-5 select-none shadow-xl">
                  ...
                  {orderBlockZones.length > 0 && (
                    <>
                      <div className="h-6 w-[1px] bg-zinc-800" />
                      <div className="flex flex-col">
                        <span className="text-[8px] text-zinc-500 font-bold uppercase tracking-wider leading-none mb-1 font-mono">ORDER BLOCKS</span>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-bold font-mono text-[#10b981]">
                            ↑{orderBlockZones.filter((z) => z.bias === 'bullish').length}
                          </span>
                          <span className="text-[10px] font-bold font-mono text-[#ef4444]">
                            ↓{orderBlockZones.filter((z) => z.bias === 'bearish').length}
                          </span>
                        </div>
                      </div>
                    </>
