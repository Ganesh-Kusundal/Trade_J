...
public final class DhanOptionsAdapter extends DhanBaseRestAdapter implements OptionsProvider {
    ...
    public RollingOptionSeries getExpiredOptionHistory(RollingOptionHistoryRequest request) {
        ...
        DhanInstrumentDefinition underlyingDefinition = resolveUnderlying(series.underlying(), series.exchangeSegment());
    ...
    public long selectStrikePaisa(
        ...
        if (resolver.isLoaded()) {
            try {
                return selectStrikeFromCatalog(underlying, exchangeSegment, spotPricePaisa, optionType, selectionKind, depth);
            } catch (IllegalArgumentException | IllegalStateException ignored) {
                // Fall back to step table when catalog lacks contracts for the underlying.
            }
        }
        return selectStrikeFromStepTable(underlying, spotPricePaisa, optionType, selectionKind, depth);
    }

    private long selectStrikeFromCatalog(
            String underlying,
            ExchangeSegment exchangeSegment,
            long spotPricePaisa,
            OptionType optionType,
        ...
        if (expiries.isEmpty()) {
            throw new IllegalStateException("No expiries for " + underlying);
        }
        LocalDate expiry = expiries.getFirst();
        List<Long> strikes = resolver.optionContracts(underlying, exchangeSegment, expiry).stream()
                .map(DhanInstrumentDefinition::strikePricePaisa)
                .filter(Objects::nonNull)
                .distinct()
                .sorted()
                .toList();
        if (strikes.isEmpty()) {
            throw new IllegalStateException("No strikes in catalog for " + underlying + " " + expiry);
        }
        return StrikeSelectionSupport.selectFromSortedStrikes(
                strikes, spotPricePaisa, optionType, selectionKind, depth);
    }

    private long selectStrikeFromStepTable(
            String underlying,
            long spotPricePaisa,
            OptionType optionType,
            StrikeSelectionKind selectionKind,
        ...
        long step = DEFAULT_STRIKE_STEPS_PAISA.getOrDefault(
                underlying == null ? "" : underlying.trim().toUpperCase(),
                DhanProtocolConstants.DEFAULT_STRIKE_STEP_PAISA
        );
